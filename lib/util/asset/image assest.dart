import 'dart:ui';

import 'package:flutter/material.dart';

class staticImages{
  //advertisment
  static String man = "asset/images/man.png";
  static String tickMark = "asset/images/tickmark.png";

  //loginScreen
  static String deliveyBoy = "asset/images/delivery1.png";

  //order
  static String lado = "asset/images/lado.png";
  static String emptyList = "asset/images/nolog.png";

  //camera
  static String gallery = "asset/images/gallery.png";
  static String camera = "asset/images/camera.png";
  static String defalutProfile = "asset/images/defaultimage.png";
}